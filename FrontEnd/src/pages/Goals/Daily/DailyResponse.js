...
export default function DailyResponse() {
  ...
  useEffect(() => {
    axios.get(`${process.env.REACT_APP_API_URL}/send-goals`, {
      headers: {
...
const prepareSubmissionData = () => {
  ...
  const submitResponse = (event) => {
    ...
    axios.get(`${process.env.REACT_APP_API_URL}/Is-Submit`, {
      headers: {
        authorization: `Bearer ${token}`,
        'content-type': 'application/json'
      }
      ...
      .then((res) => {
        if (res.data.message === 'False') {
          ...
          // Submit to backend
          axios.post(`${process.env.REACT_APP_API_URL}/Submit-Response`, submissionData, {
            headers: {
              authorization: `Bearer ${token}`,
              'content-type': 'application/json'
            }
        ...
        <main>
          ...
          {aiGenQues.length === 0 ? (
            <div className="DailyGoalMess">
              ...
              <span>Today's goals not submitted yet</span>
            </div>
          ) : (
            <>
